...
def test_ingestion_partial_accept_persists_valid_and_rejected_records(db_session: Session) -> None:
    ...
    raw_count = db_session.scalar(select(func.count()).select_from(RawListing))
    rejected_count = db_session.scalar(select(func.count()).select_from(RejectedListing))
...
def test_ingestion_dedup_upserts_duplicate_records(db_session: Session) -> None:
...
def test_ingestion_allows_land_records_with_missing_bedbath(
    db_session: Session,
    tmp_path: Path,
) -> None:
    payload = [
